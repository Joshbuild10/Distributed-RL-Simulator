...
def intellect3() -> Scenario:
    ...
    return Scenario(
        ...
        model=GLM45_AIR,
        # E[R]=24k: high end of measured single-turn reasoning (DeepSeek-R1 max ~26k); cv 0.5 from measured math/code reasoning traces.
        rl=RLSpec(prompts_per_batch=256, responses_per_prompt=16,
                  prompt_len=1000, response_len_mean=24000.0, response_len_cv=0.5,
        ...
        inf_hw=HWSpec("44x8 H200 (vLLM, TP=8)", n_nodes=44, mfu=0.4,
...
def primerl_paper() -> Scenario:
    ...
    return Scenario(
        ...
        inf_hw=HWSpec("4x TP=4 vLLM replicas (16 H200)", n_nodes=4, gpus_per_node=4,
                      mfu=0.4, bw_eff=0.85, **INF_REPLICA),
...
def primerl_deepdive() -> Scenario:
    ...
    return Scenario(
        ...
        train_hw=HWSpec("1x8 H200", n_nodes=1, mfu=0.40, penalty_para=1,
                        n_shard=8, **H200_NODE),
        inf_hw=HWSpec("1x8 H200", n_nodes=1, mfu=0.4, bw_eff=0.85, **H200_NODE),
...
def _areal(model: ModelSpec, n_nodes_total: int, steps: int, total_hours: float,
    ...
    return Scenario(
        ...
        inf_hw=HWSpec(f"{inf_gpus} H800 (TP={inf_tp})", n_nodes=inf_gpus // inf_tp,
                      gpus_per_node=inf_tp, mfu=0.4, bw_eff=0.85, **inf_node),
...
# E[R],cv set from MEASURED reasoning-trace distributions:
