...
def intellect2(target: str = "short") -> Scenario:
    ...
    """
    if target == "short":
        rmean, cv, steps, tstep = 2500.0, 0.0, 200, 22 * 60
    else:
        rmean, cv, steps, tstep = 6000.0, 0.0, 350, 21 * 60
    ...
    return Scenario(
        ...
        inf_hw=HWSpec("decentralised swarm", n_nodes=8, mfu=0.4, bw_eff=0.85,
...
def intellect3() -> Scenario:
    """INTELLECT-3 (https://arxiv.org/pdf/2512.16144). Centralised 512xH200 cluster, prime-rl.
    ...
    return Scenario(
        ...
        model=GLM45_AIR,
        rl=RLSpec(prompts_per_batch=256, responses_per_prompt=16,
                  prompt_len=1000, response_len_mean=22000.0, response_len_cv=0.5,
...
def primerl_paper() -> Scenario:
    ...
    return Scenario(
        ...
        rl=RLSpec(prompts_per_batch=128, responses_per_prompt=16,
                  prompt_len=100, response_len_mean=7470.0, response_len_cv=0.5,
...
# model's home regime (disaggregated async). Architectures confirmed from HF
# config.json for DeepSeek-R1-Distill-Qwen-{1.5B,7B,14B,32B} (Qwen2.5 backbones).
...
R1_QWEN_7B = ModelSpec(          # Qwen2.5-Math-7B backbone
    ...
    vocab=152064, tied_embeddings=True)
...
def _areal(model: ModelSpec, n_nodes_total: int, steps: int, total_hours: float,
           er_mean: float, inf_tp: int, task: str) -> Scenario:
    """One AReaL run. Disaggregated: 1/4 of nodes train, 3/4 infer (async, in-flight). Had higher throughput that 1:1 allocation
    ...
    return Scenario(
        ...
        rl=RLSpec(prompts_per_batch=512, responses_per_prompt=16,
                  prompt_len=1000, response_len_mean=er_mean, response_len_cv=0.5,
        ...
        inf_hw=HWSpec(f"{inf_gpus} H800 (TP={inf_tp})", n_nodes=inf_gpus // inf_tp,
                      gpus_per_node=inf_tp, mfu=0.35, bw_eff=0.85, **inf_node),
        ...
        notes=f"AReaL async disaggregated PPO; {task}. E[R]={er_mean:,.0f} ASSUMED (not published).",
        published=dict(t_step=per_step, t_total=total_hours * 60 * 60),
    )


def areal_1_5b(): return _areal(R1_QWEN_1_5B, 16, 250, 14.8, 12000.0, inf_tp=1, task="math")
def areal_7b():   return _areal(R1_QWEN_7B,   24, 250, 25.4, 12000.0, inf_tp=1, task="math")
def areal_14b():  return _areal(R1_QWEN_14B,  32,  80, 21.9, 12000.0, inf_tp=2, task="code")
def areal_32b():  return _areal(R1_QWEN_32B,  48,  60, 31.1, 12000.0, inf_tp=4, task="code")
